...
class OnlineBookstore
{
    ...
    public void Logout()
    {
        ...
        currentUser = new User("guest", "");
        Console.ForegroundColor = ConsoleColor.Green;
        ConsoleUtils.PrintTextWithBorder("You have been logged out.");
        Console.ResetColor();
        Console.ReadKey();
    }
    ...
    public void ProcessOrder(User user, ShoppingCart cart)
    {
        ...
        SaveInventoryToFile();
        SaveOrderHistoryToFile(user);
    }
    private void SaveOrderHistoryToFile(User user)
    {
        ...
        try
        {
            using (StreamWriter sw = new StreamWriter(filePath, true))
            {
                ...
                {
                    sw.WriteLine($"{user.Username},{order.OrderId},{order.TotalCost},{order.OrderDate}");
                }
            }
        }
        catch (Exception ex)
        {
            ConsoleUtils.PrintCentered($"Error writing order history to file: {ex.Message}");
        }
    }
    private void LoadOrderHistoryForUser(User user)
    {
        ...
        {
            ConsoleUtils.PrintCentered($"Error loading order history from file: {ex.Message}");
        }
    }
